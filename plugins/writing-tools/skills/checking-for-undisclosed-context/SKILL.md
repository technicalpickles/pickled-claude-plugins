...
3. **Flag three shapes of leak:**
   ...
   - **Borrowed structure** - the text's organization only makes sense if you already know the shape of the thing it describes (a section is legible only if you know the pipeline has five stages, because it's silently structured around them).
